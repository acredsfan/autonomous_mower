...
class TestAvoidanceAlgorithmProperties:
    ...
    def test_obstacle_position_estimation_properties(self, current_position, heading, distance, obstacle_angle):
        ...
        assert len(
            obstacle_position) == 2, "Obstacle position should be a tuple of (lat, lng)"
        ...
        calculated_distance = math.sqrt(
            lat_diff**2 + lng_diff**2) * 111000  # Convert to meters
    ...
    def test_path_modification_properties(self, path, obstacles):
        ...
        if obstacle_indices:
            assert len(modified_path) != len(
                path), "Modified path should have a different length"
    ...
    def test_path_smoothing_properties(self, path):
        ...
        assert len(smoothed_path) >= len(
            path), "Smoothed path should have at least as many points"
        ...
        for i in range(1, len(smoothed_path) - 1):
            ...
            assert angle < np.pi / \
                2, f"Smoothed path has a sharp turn at point {i}"
    ...
    def test_sensor_data_processing_properties(self, sensor_data):
        ...
        for obstacle in obstacles:
            assert hasattr(
                obstacle, 'position'), "Obstacle should have a position"
            assert hasattr(obstacle, 'size'), "Obstacle should have a size"
            assert hasattr(
                obstacle, 'confidence'), "Obstacle should have a confidence"

            assert len(
                obstacle.position) == 2, "Obstacle position should be a tuple of (x, y)"

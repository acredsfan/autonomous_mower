...
def test_gps_sensor(sim_world, sim_gps):
    ...
    if data["fix_quality"] > 0:
        ...
        ), "Position should not be None when we have a GPS fix"
...
def test_imu_sensor(sim_world, sim_imu):
    ...
    assert (
        abs(orientation[2] - robot_heading) < 0.1
    ), "IMU yaw should be close to robot heading"
...
def test_tof_sensor(sim_world, sim_tof):
    ...
    if front_distance is not None:
        ...
        assert (
            3.0 < front_distance < 7.0
        ), f"Front distance should be about 5m, got {front_distance}m"
...
def test_blade_controller(sim_world, sim_blade_controller):
    ...
    assert sim_world.get_robot_state()[
        "blade_running"
    ], "Blade should be on after setting state to True"
    ...
    assert not sim_world.get_robot_state()[
        "blade_running"
    ], "Blade should be off after setting state to False"

    # Log the blade state
    logger.info(
        f"Blade state: {sim_world.get_robot_state()['blade_running']}"
    )
...
def test_motor_controller(sim_world, sim_motor_controller):
    ...
    assert (
        initial_state["motor_speeds"][0] == 0.0
    ), "Left motor should initially be stopped"
    assert (
        initial_state["motor_speeds"][1] == 0.0
    ), "Right motor should initially be stopped"

    # Set the motor speeds
    sim_motor_controller.set_motor_speeds(
        0.5, 0.5
    )  # 50% speed on both motors
    ...
    assert (
        motor_state["motor_speeds"][0] == 0.5
    ), "Left motor should be at 50% speed"
    assert (
        motor_state["motor_speeds"][1] == 0.5
    ), "Right motor should be at 50% speed"
    ...
    assert (
        final_state["motor_speeds"][0] == 0.0
    ), "Left motor should be stopped"
    assert (
        final_state["motor_speeds"][1] == 0.0
...
def test_all_hardware(sim_world, sim_hardware):
    ...
    assert (
        robot_state["motor_speeds"][0] == 0.5
    ), "Left motor should be at 50% speed"
    assert (
        robot_state["motor_speeds"][1] == 0.5
    ), "Right motor should be at 50% speed"

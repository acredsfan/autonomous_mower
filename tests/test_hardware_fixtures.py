...
def test_imu_sensor(sim_world, sim_imu):
    ...
    # Set up the virtual world
    sim_world.set_robot_position(Vector2D(10.0, 10.0), 0.5)  # 0.5 radians heading
    ...
    assert abs(orientation[2] - robot_heading) < 0.1, "IMU yaw should be close to robot heading"
...
def test_blade_controller(sim_world, sim_blade_controller):
    ...
    assert not sim_world.get_robot_state()["blade_running"], "Blade should initially be off"
    ...
    assert sim_world.get_robot_state()["blade_running"], "Blade should be on after setting state to True"
    ...
    assert not sim_world.get_robot_state()["blade_running"], "Blade should be off after setting state to False"

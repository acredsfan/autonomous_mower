...
class TestAddressToCoordinates(unittest.TestCase):
    ...
    def test_api_key_missing_returns_none_logs_error(self, mock_logger_error):
        ...
        mock_logger_error.assert_called_with(
            "Cannot perform geocoding: GOOGLE_MAPS_API_KEY is not set."
        )
    ...
    def test_request_denied_returns_none_logs_error(self, mock_logger_error, mock_urlopen):
        ...
        self.assertIsNone(result)
        mock_logger_error.assert_called_with(
            "Geocoding API request denied. Verify API key and permissions."
        )
    ...
    def test_over_query_limit_returns_none_logs_error(self, mock_logger_error, mock_urlopen):
        ...
        self.assertIsNone(result)
        mock_logger_error.assert_called_with(
            "Geocoding API query limit exceeded. Check usage and billing."
        )
...
class TestGetLocationFromWifiCell(unittest.TestCase):
    ...
    def _mock_urlopen_post_response(self, mock_urlopen_patch, status_code=200, response_dict=None, http_error_code=None, http_error_reason=None, read_side_effect=None):
        mock_response = MagicMock()
    ...
    SAMPLE_CELL = [{"cellId": 42, "locationAreaCode": 415, "mobileCountryCode": 310, "mobileNetworkCode": 410, "age": 0, "signalStrength": -60, "timingAdvance": 15}]
    SUCCESS_RESPONSE = {"location": {"lat": 51.0, "lng": -0.1}, "accuracy": 1200.0}
    NOT_FOUND_ERROR_RESPONSE_BODY = {"error": {"errors": [{"domain": "geolocation", "reason": "notFound", "message": "Not Found"}], "code": 404, "message": "Not Found"}}
    ...
    def test_success_with_wifi_and_cell(self, mock_urlopen):
        ...
        self.assertEqual(result, self.SUCCESS_RESPONSE)
        self.assertEqual(json.loads(mock_urlopen.call_args[0][0].data), {"wifiAccessPoints": self.SAMPLE_WIFI, "cellTowers": self.SAMPLE_CELL})
    ...
    @patch('mower.navigation.gps.urllib.request.urlopen')
    @patch.object(logger_under_test, 'warning')
    def test_api_returns_not_found_error_in_body(self, mock_logger, mock_urlopen): # API returns 200 OK, but error in body
        importlib.reload(gps)
    ...
    def test_api_returns_http_403_key_error(self, mock_logger, mock_urlopen):
        importlib.reload(gps)
        error_response_body = {"error": {"code": 403, "message": "API key invalid", "errors": [{"reason": "keyInvalid"}]}}
        self._mock_urlopen_post_response(mock_urlopen, http_error_code=403, http_error_reason="Forbidden", response_dict=error_response_body)
    ...
    def test_unexpected_error(self, mock_logger_error, mock_urlopen):
        ...
        mock_logger_error.assert_called_with("Unexpected error during Geolocation API call: Completely unexpected error", exc_info=True)

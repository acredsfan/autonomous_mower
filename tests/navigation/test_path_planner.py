...
class TestPathPlannerElevation(unittest.TestCase):
    def setUp(self):
        ...
        self.learning_config = LearningConfig(
            model_path="test_model.json", update_frequency=10000, memory_size=10, batch_size=2
        )
    ...
    def test_api_key_missing_logs_warning_on_import_and_usage(self, mock_logger_warning):
        ...
        mock_logger_warning.assert_any_call("Cannot fetch elevation data: GOOGLE_MAPS_API_KEY is not set.")
    ...
    def test_get_elevation_success(self, mock_requests_get):
        with patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": "fake_key"}, clear=True):
            ...
            mock_response.json.return_value = {
                "results": [{"elevation": 10.0}, {"elevation": 20.0}],
                "status": "OK",
            }
    ...
    def _run_get_elevation_test_with_mocked_api(
        self, api_response_json, expected_log_message, mock_logger_error, mock_requests_get
    ):
        with patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": "fake_key"}, clear=True):
            import importlib
    ...
    def _run_get_elevation_test_with_request_exception(
        self, exception_to_raise, expected_log_message, mock_logger_error, mock_requests_get
    ):
        with patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": "fake_key"}, clear=True):
            import importlib
    ...
    def test_get_elevation_batched_requests(self, mock_requests_get):
        with patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": "fake_key"}, clear=True):
            ...
            mock_response_1.json.return_value = {
                "results": [{"elevation": float(i)} for i in range(512)],
                "status": "OK",
            }
            mock_response_2 = MagicMock()
            mock_response_2.json.return_value = {
                "results": [{"elevation": float(i)} for i in range(512, num_points)],
                "status": "OK",
            }
    ...
    def test_generate_path_with_elevation_data(self, mock_get_elevation):
        ...
        with (
            patch.object(
                self.planner, "_generate_pattern_path", return_value=fixed_path_coords
            ) as mock_instance_generate_pattern,
    ...
    def test_reward_calculation_with_elevation(self):
        ...
        with (
            patch.object(self.planner, "_calculate_path_distance", return_value=20.0),
    ...
    def test_reward_calculation_with_steep_slope_penalty(self):
        ...
        with (
            patch.object(self.planner, "_calculate_path_distance", return_value=10.0),
            patch.object(self.planner, "_calculate_coverage", return_value=0.8),

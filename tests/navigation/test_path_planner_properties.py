...
def pattern_config_strategy(draw):
    ...
    angles = sorted([draw(st.floats(min_value=0, max_value=2*np.pi))
...
class TestPathPlannerProperties:
    ...
    def test_path_within_boundary(self, pattern_config):
        ...
        for point in path:
            assert self._point_near_or_in_polygon(point, pattern_config.boundary_points), \

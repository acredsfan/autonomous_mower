...
class TestConfigComponentInitialization:
    ...
    def test_resource_manager_initialization_from_config(
        ...
        with patch(
            "mower.mower.ResourceManager._initialize_hardware"
        ) as mock_init_hardware:
            with patch(
                "mower.mower.ResourceManager._initialize_software"
            ) as mock_init_software:
                with patch("mower.mower.get_config") as mock_get_config:
                    # Configure the mock to return values from the actual config manager
                    mock_get_config.side_effect = (
                        lambda key, default=None: config_manager.get(
                            key, default
                        )
                    )

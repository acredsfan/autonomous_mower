...
class AvoidanceAlgorithm:
    ...
    def __init__(self, cfg):
        ...
        self.q_table = np.zeros(
            (path_planning.GRID_SIZE[0],
             path_planning.GRID_SIZE[1],
             4))  # 4 directions
    ...
    def q_learning(
        ...
        if self.last_action is not None:
            ...
            old_value = self.q_table[current_state[0],
                                     current_state[1], self.last_action]
            next_max = np.max(
    ...
    def get_current_state(self):
        ...
        grid_cell = self.path_planning.coord_to_grid(
            current_position[0], current_position[1])
        return grid_cell

    def run_avoidance(self):
        """Continuously run the avoidance algorithm using
        data from ToF sensors and the camera."""
        tof_thread = threading.Thread(
            target=self._tof_avoidance_thread, daemon=True)
        tof_thread.start()
        ...
        try:
            while True:
                ...
                if (self.tof_avoidance.obstacle_left or
                        self.tof_avoidance.obstacle_right or

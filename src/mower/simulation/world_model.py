...
class Robot:
    ...
    def __init__(self, position: Vector2D = Vector2D(0, 0), heading: float = 0.0):
        ...
        self.motor_speeds = [0.0, 0.0]  # Left and right motor speeds (-1.0 to 1.0)
    ...
    def _update_battery(self, dt: float) -> None:
        ...
        motor_current = sum(abs(speed) for speed in self.motor_speeds) * 2.0  # 2A per motor at full speed

...
def state_changed(control):
    ...
    if DEBUG:
        logger.debug("%f\t%s (%i): %i (%i)" %
                    (time.monotonic(), control.name, len(control.channel), 
                     control.value, servo_duty_cycle(control.value)))
...
# Using project-specific pin assignments
...
def main():
    ...
    while True:
        ...
        if DEBUG:
            logger.info("Get: steering=%i, throttle=%i" %

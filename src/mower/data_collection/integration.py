...
from mower.ui.web_ui.data_collection_controller import (
...
def integrate_data_collection(app: Flask, mower: Any) -> bool:
    ...
    try:
        ...
        storage_path = config_manager.get_config(
            "data_collection", {}).get("storage_path", "data/collected_images")
            ...
            if not os.path.exists(images_symlink) and storage_path:
                try:
                    ...
                    if not os.path.isabs(storage_path):
                        abs_storage_path = os.path.join(
                            os.path.dirname(app.root_path), storage_path)
                    else:
                    ...
                    if os.name != 'nt':  # Not Windows
                        ...
                        try:
                            ...
                            subprocess.run(
                                ['mklink', '/J', images_symlink,
                                 abs_storage_path],
                                shell=True, check=True)
                        except (ImportError, subprocess.SubprocessError):
                            logger.warning(
                                "Could not create symlink on Windows. "
                                "Images may not be accessible via the web UI.")

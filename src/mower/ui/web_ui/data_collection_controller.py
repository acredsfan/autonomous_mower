...
datacollection_bp = Blueprint("datacollection", __name__, url_prefix="/datacollection")
...
class DataCollectionController:
    ...
    def _register_routes(self) -> None:
        ...
        datacollection_bp.route("/sessions/<session_id>")(self.get_session_details)

        # POST routes
        datacollection_bp.route("/start", methods=["POST"])(self.start_collection)
        datacollection_bp.route("/stop", methods=["POST"])(self.stop_collection)
        datacollection_bp.route("/pattern", methods=["POST"])(self.change_pattern)
        datacollection_bp.route("/interval", methods=["POST"])(self.set_interval)

    def index(self):
        """Render data collection UI page."""
        return render_template(
            "datacollection.html", title="Data Collection", status=self.data_collector.get_session_status()
        )
    ...
    def start_collection(self):
        ...
        if session_id:
            ...
            return jsonify({"success": False, "message": "Failed to start data collection"}), 500
    ...
    def stop_collection(self):
        ...
        if stats.get("status") == "not_running":
            return jsonify({"success": False, "message": "No data collection session in progress"}), 400

        return jsonify({"success": True, "message": "Data collection stopped", "stats": stats})
    ...
    def change_pattern(self):
        ...
        if not pattern:
            return jsonify({"success": False, "message": "Pattern name is required"}), 400

        success = self.data_collector.change_collection_pattern(pattern)

        if success:
            return jsonify({"success": True, "message": f"Pattern changed to {pattern}"})
        else:
            return jsonify({"success": False, "message": f"Failed to change to pattern {pattern}"}), 400
    ...
    def set_interval(self):
        ...
        try:
            ...
            if interval <= 0:
                return jsonify({"success": False, "message": "Interval must be positive"}), 400

            self.data_collector.set_image_interval(interval)

            return jsonify({"success": True, "message": f"Image capture interval set to {interval} seconds"})
    ...
    def get_sessions(self):
        ...
        try:
                    ...
                    if os.path.isdir(full_path):
                        ...
                        if os.path.exists(info_path):
                            try:
                                ...
                                logger.error(f"Error reading session info {info_path}: {e}")
            ...
            return jsonify({"success": False, "message": f"Error retrieving sessions: {str(e)}", "sessions": []}), 500
        ...
        try:
            ...
            for file in os.listdir(session_path):
                if file.endswith(".jpg") or file.endswith(".jpeg"):
                    ...
                    rel_path = os.path.join("data/collected_images", os.path.basename(session_path), file)
                    image_list.append({"name": file, "path": rel_path.replace("\\", "/")})
            ...
            return jsonify({"success": True, "session_id": session_id, "details": session_info})

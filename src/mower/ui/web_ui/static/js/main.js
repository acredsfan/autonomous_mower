...
function updateSensorData(data) {
  ...
  if (data.environment) {
    ...
    if (tempElement && data.environment.temperature !== undefined) {
      const temp = parseFloat(data.environment.temperature);
      tempElement.textContent = isNaN(temp) ? "--.-" : temp.toFixed(1) + "°C";
    }

    // Update humidity
    const humElement = document.getElementById("sensor_humidity");
    if (humElement && data.environment.humidity !== undefined) {
      const hum = parseFloat(data.environment.humidity);
      humElement.textContent = isNaN(hum) ? "--.-" : hum.toFixed(1) + "%";
    }

    // Update pressure
    const pressElement = document.getElementById("sensor_pressure");
    if (pressElement && data.environment.pressure !== undefined) {
      const press = parseFloat(data.environment.pressure);
      pressElement.textContent = isNaN(press)
        ? "----"
        : press.toFixed(0) + "hPa";
    }
  }
  ...
  if (data.distance) {
    ...
    if (leftDistElement && data.distance.left !== undefined) {
      const leftDist = parseFloat(data.distance.left);
      leftDistElement.textContent = isNaN(leftDist)
        ? "--.-"
        : leftDist.toFixed(1) + "cm";
    }

    // Update right distance
    const rightDistElement = document.getElementById("sensor_rightDistance");
    if (rightDistElement && data.distance.right !== undefined) {
      const rightDist = parseFloat(data.distance.right);
      rightDistElement.textContent = isNaN(rightDist)
        ? "--.-"
        : rightDist.toFixed(1) + "cm";
    }
  }
...
function updatePositionData(data) {
  ...
  // Extract position data - handle both array format and object format
  let latitude = null;
  let longitude = null;

  // Check if data has GPS info
  if (data.gps) {
    if (data.gps.latitude !== undefined && data.gps.longitude !== undefined) {
      latitude = parseFloat(data.gps.latitude);
      longitude = parseFloat(data.gps.longitude);
    }
  }

  // If no GPS data, check for currentPosition format
  if (latitude === null && data.currentPosition) {
    if (
      Array.isArray(data.currentPosition) &&
      data.currentPosition.length >= 2
    ) {
      latitude = parseFloat(data.currentPosition[0]);
      longitude = parseFloat(data.currentPosition[1]);
    } else if (typeof data.currentPosition === "object") {
      latitude = parseFloat(
        data.currentPosition.latitude || data.currentPosition.lat || 0
      );
      longitude = parseFloat(
        data.currentPosition.longitude || data.currentPosition.lng || 0
      );
    }
  }
  ...
  if (latElement && latitude !== null) {
    latElement.textContent = latitude.toFixed(6);
  }

  if (lngElement && longitude !== null) {
  ...
  const mapData = {
    ...
    homePosition: data.homePosition || null,
    waypoints: data.waypoints || [],
  };

  // Update map if it exists and the updateMap function is available
  if (typeof updateMap === "function" && mapData.currentPosition) {
    updateMap(mapData);
  }
}

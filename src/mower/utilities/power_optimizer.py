...
class PowerOptimizer:
    ...
    def _update_power_mode(self, battery_level: float):
        ...
        if new_mode != self.power_mode:
            logger.info(f"Changing power mode from {self.power_mode.name} to {new_mode.name} (battery: {battery_level}%)")
    ...
    def _set_cpu_governor(self, governor: str):
        ...
        try:
            ...
            if os.name == 'posix':
                ...
                logger.warning(f"Setting CPU governor not supported on {os.name}")
        except Exception as e:
    ...
    def _apply_component_settings(self, component_name: str, settings: Dict[str, Any]):
        ...
        try:
            ...
            if hasattr(component, 'set_poll_interval') and 'sensor_poll_interval' in settings:
                ...
                logger.debug(f"Set poll interval for {component_name} to {settings['sensor_poll_interval']}s")
            
            if hasattr(component, 'set_resolution') and 'camera_resolution' in settings:
                component.set_resolution(settings['camera_resolution'])
                logger.debug(f"Set resolution for {component_name} to {settings['camera_resolution']}")
            ...
            if hasattr(component, 'set_update_interval') and 'gps_update_interval' in settings:
                ...
                logger.debug(f"Set update interval for {component_name} to {settings['gps_update_interval']}s")
            
            if hasattr(component, 'set_background_processes') and 'background_processes' in settings:
                component.set_background_processes(settings['background_processes'])
                logger.debug(f"Set background processes for {component_name} to {settings['background_processes']}")
        except Exception as e:
...
def optimize_power_consumption():
    ...
    try:
        ...
        ObstacleDetector.detect_obstacles = power_aware(power_aware_detect_obstacles)

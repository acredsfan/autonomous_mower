...
class BNO085Sensor:
    ...
    serial connection, data parsing, and provides thread-safe access to sensor data.
    ...
    def __init__(self, serial_port_name=None, baudrate=115200):
        ...
        Args:
            serial_port_name (str, optional): Serial port name. If None, auto-discovery
                                             is attempted. Defaults to None.
    ...
    def _enable_rotation_vector(self):
        ...
        Troubleshooting:
            - If no rotation data is received, check if sensor needs calibration
                    ...
                    if self.serial_port.in_waiting > 0:
                        ...
                        while len(buffer) >= 4:  # Min packet size is 4 bytes
                            ...
                            if len(buffer) >= length + 4:
                                ...
                                buffer = buffer[length+4:]  # Remove processed packet
                            else:
    ...
    def _process_packet(self, channel, packet):
        ...
        if channel == CHANNEL_REPORTS and report_id == SENSOR_REPORTID_ROTATION_VECTOR:
            ...
            if len(packet) >= 17:
                with self.lock:
                    # Extract quaternion components (i, j, k, real) - BNO085 specific format
                    ...
                    # BNO085 uses different quaternion order, rearrange to w, x, y, z
                    ...
                    if len(packet) >= 18:
                        ...
                        self.calibration_status = min(3, int(accuracy / 30))  # Convert to 0-3 scale
                    
        elif channel == CHANNEL_REPORTS and report_id == 0x14:  # Accelerometer report
        ...
        elif channel == CHANNEL_REPORTS and report_id == 0x13:  # Gyroscope report
    ...
    def _update_euler_angles(self):
        ...
        in degrees. This provides a more intuitive representation of orientation.

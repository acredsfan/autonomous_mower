...
class SensorFallbackManager:
    ...
    def register_sensor(
        ...
        logger.info((f"Registered sensor {sensor_id} in group {group_id} with" f" priority {priority.name}"))
        ...
        with self._locks["sensors"]:
            ...
            for fallback_id in fallback_sensors:
                if fallback_id not in self._sensors:
                    logger.error(f"Cannot set fallback chain: Fallback sensor " f"{fallback_id} not registered")
                    return
    ...
    def _update_active_sensors(self) -> None:
        ...
        with (
            ...
            for group_id, sensor_ids in self._sensor_groups.items():
                ...
                if current_active in sensor_ids and self._sensor_status.get(current_active) == SensorStatus.OPERATIONAL:
                    continue
                ...
                if best_sensor:
                    if best_sensor != current_active:
                        logger.info(
                            f"Switching active sensor for group {group_id} " f"from {current_active} to {best_sensor}"
                        )
    ...
    def _find_best_sensor(self, group_id: str, sensor_ids: List[str]) -> Optional[str]:
        ...
        for priority in SensorPriority:
            for sensor_id in sensor_ids:
                if (
                    ...
                    and self._sensor_status.get(sensor_id) == SensorStatus.OPERATIONAL
                ):
                    return sensor_id
        ...
        for priority in SensorPriority:
            for sensor_id in sensor_ids:
                if (
                    self._sensor_priorities.get(sensor_id) == priority
                    and self._sensor_status.get(sensor_id) == SensorStatus.DEGRADED
                ):
                    return sensor_id
...
class SensorCalibrationManager:
    ...
    def register_calibration_procedure(self, sensor_id: str, procedure: Callable) -> None:
        ...
        with self._locks["sensors"]:
            if sensor_id not in self._sensors:
                logger.error((f"Cannot register calibration procedure: Sensor " f"{sensor_id} not registered"))
                return
    ...
    def calibrate_sensor(self, sensor_id: str) -> bool:
        ...
        with self._locks["sensors"], self._locks["status"]:
            ...
            if sensor_id not in self._calibration_procedures:
                logger.error((f"Cannot calibrate: No calibration procedure for" f" sensor {sensor_id}"))
                return False
        ...
        threading.Thread(target=self._run_calibration, args=(sensor_id,), daemon=True).start()

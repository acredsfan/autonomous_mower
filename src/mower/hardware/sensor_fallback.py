...
class SensorFallbackManager:
    ...
    def __init__(self):
        ...
        self._fallback_chains = {}  # Dict of sensor_id -> list of fallback sensor_ids
    ...
    def register_sensor(self, sensor_id: str, sensor: SensorInterface, 
                       group_id: str, priority: SensorPriority) -> None:
        """
        ...
        with self._locks['sensors']:
            ...
            with self._locks['active']:
                if group_id not in self._active_sensors:
                    ...
                    if self._sensor_priorities[sensor_id].value < self._sensor_priorities[current_active].value:
                        self._active_sensors[group_id] = sensor_id
        
        logger.info(f"Registered sensor {sensor_id} in group {group_id} with priority {priority.name}")
    
    def set_fallback_chain(self, sensor_id: str, fallback_sensors: List[str]) -> None:
        """
        ...
        with self._locks['sensors']:
            if sensor_id not in self._sensors:
                logger.error(f"Cannot set fallback chain: Sensor {sensor_id} not registered")
                return
            
            # Verify all fallback sensors are registered
            for fallback_id in fallback_sensors:
                if fallback_id not in self._sensors:
                    logger.error(f"Cannot set fallback chain: Fallback sensor {fallback_id} not registered")
                    return
    ...
    def _check_sensor_health(self, sensor_id: str, sensor: SensorInterface) -> SensorStatus:
        """
    ...
    def _update_active_sensors(self) -> None:
        ...
        with self._locks['sensors'], self._locks['status'], self._locks['active']:
            for group_id, sensor_ids in self._sensor_groups.items():
                ...
                if (current_active in sensor_ids and 
                    self._sensor_status.get(current_active) == SensorStatus.OPERATIONAL):
                    continue
                ...
                if best_sensor:
                    if best_sensor != current_active:
                        logger.info(f"Switching active sensor for group {group_id} from {current_active} to {best_sensor}")
                        self._active_sensors[group_id] = best_sensor
                else:
                    logger.warning(f"No operational sensors available for group {group_id}")
    
    def _find_best_sensor(self, group_id: str, sensor_ids: List[str]) -> Optional[str]:
        """
        ...
        for priority in SensorPriority:
            for sensor_id in sensor_ids:
                if (self._sensor_priorities.get(sensor_id) == priority and 
                    self._sensor_status.get(sensor_id) == SensorStatus.OPERATIONAL):
                    return sensor_id
        ...
        for priority in SensorPriority:
            for sensor_id in sensor_ids:
                if (self._sensor_priorities.get(sensor_id) == priority and 
                    self._sensor_status.get(sensor_id) == SensorStatus.DEGRADED):
                    return sensor_id
        ...
        if current_active in sensor_ids:
            ...
            if status in [SensorStatus.CALIBRATING, SensorStatus.INITIALIZING]:
    ...
    def get_active_sensor_instance(self, group_id: str) -> Optional[SensorInterface]:
        """
...
class SensorCalibrationManager:
    ...
    def __init__(self):
        ...
        self._sensors = {}  # Dict of sensor_id -> sensor instance
        self._calibration_procedures = {}  # Dict of sensor_id -> calibration procedure
        self._calibration_status = {}  # Dict of sensor_id -> calibration status
        self._locks = {
            'sensors': threading.Lock(),
            'status': threading.Lock()
        }
    ...
    def register_sensor(self, sensor_id: str, sensor: SensorInterface) -> None:
        """
    ...
    def register_calibration_procedure(self, sensor_id: str, procedure: Callable) -> None:
        """
        ...
        with self._locks['sensors']:
            if sensor_id not in self._sensors:
                logger.error(f"Cannot register calibration procedure: Sensor {sensor_id} not registered")
                return
            
            self._calibration_procedures[sensor_id] = procedure
        
        logger.info(f"Registered calibration procedure for sensor {sensor_id}")
    ...
    def calibrate_sensor(self, sensor_id: str) -> bool:
        ...
        with self._locks['sensors'], self._locks['status']:
            if sensor_id not in self._sensors:
                logger.error(f"Cannot calibrate: Sensor {sensor_id} not registered")
                return False
            
            if sensor_id not in self._calibration_procedures:
                logger.error(f"Cannot calibrate: No calibration procedure for sensor {sensor_id}")
                return False
            
            if self._calibration_status[sensor_id]['in_progress']:
                logger.warning(f"Calibration already in progress for sensor {sensor_id}")
                return False
            
            # Update status
            self._calibration_status[sensor_id]['in_progress'] = True
            self._calibration_status[sensor_id]['success'] = False
            self._calibration_status[sensor_id]['message'] = "Calibration in progress"
        ...
        try:
            ...
            with self._locks['status']:
                if result:
                    ...
                    self._calibration_status[sensor_id]['message'] = "Calibration successful"
                    self._calibration_status[sensor_id]['last_calibration'] = datetime.now()
                    self._calibration_status[sensor_id]['calibration_needed'] = False
                else:
                    self._calibration_status[sensor_id]['success'] = False
                    self._calibration_status[sensor_id]['message'] = "Calibration failed"
                
                self._calibration_status[sensor_id]['in_progress'] = False
            
            logger.info(f"Calibration for sensor {sensor_id} completed with result: {result}")
        except Exception as e:
            logger.error(f"Error during calibration of sensor {sensor_id}: {e}")
            with self._locks['status']:
                self._calibration_status[sensor_id]['in_progress'] = False
                self._calibration_status[sensor_id]['success'] = False
                self._calibration_status[sensor_id]['message'] = f"Calibration error: {str(e)}"
    ...
    def get_all_calibration_statuses(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        with self._locks['status']:
            return {sensor_id: status.copy() for sensor_id, status in self._calibration_status.items()}
...
def calibrate_imu(sensor):
    ...
    try:
        ...
        logger.info("Please place the mower on a level surface and press Enter to continue...")
...
def calibrate_distance_sensor(sensor):
    ...
    try:
        ...
        logger.info("Please clear the area in front of the sensor and press Enter to continue...")
        # In a real implementation, this would wait for user input or a signal
        time.sleep(2)  # Simulating wait for user action

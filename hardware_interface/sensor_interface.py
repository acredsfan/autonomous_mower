        ...
        while not self.stop_thread:
            try:
                with self.sensor_data_lock:
                    ...
                    self.sensor_data['heading'] = self.read_sensor_data(self.sensors['bno085'], BNO085Sensor.calculate_heading, "BNO085 Heading")
                    self.sensor_data['pitch'] = self.read_sensor_data(self.sensors['bno085'], BNO085Sensor.calculate_pitch, "BNO085 Pitch")
                    self.sensor_data['roll'] = self.read_sensor_data(self.sensors['bno085'], BNO085Sensor.calculate_roll, "BNO085 Roll")
                    self.sensor_data['speed'] = self.read_sensor_data(self.sensors['bno085'], BNO085Sensor.calculate_speed, "BNO085 Speed")

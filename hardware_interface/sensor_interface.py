...
class SensorInterface:
    ...
    def init_sensors(self):
        ...
        self.sensors = {
            'bme280': self.initialize_sensor(
                BME280Sensor.init_bme280, "BME280"), 'bno085': self.initialize_sensor(
                lambda: BNO085Sensor.init_bno085(
                    self.i2c), "BNO085"), 'ina3221': self.initialize_sensor(
                    lambda: INA3221Sensor.init_ina3221(
                        self.i2c), "INA3221"), 'vl53l0x': self.initialize_sensor(
            ...
            try:
                with self.sensor_data_lock:
                    self.sensor_data['bme280'] = self.read_sensor_data(
                        self.sensors['bme280'], BME280Sensor.read_bme280, "BME280")
                    self.sensor_data['accel'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.read_bno085_accel, "BNO085 Accelerometer")
                    self.sensor_data['gyro'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.read_bno085_gyro, "BNO085 Gyroscope")
                    ...
                    self.sensor_data['heading'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.calculate_heading, "BNO085 Heading")
                    self.sensor_data['pitch'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.calculate_pitch, "BNO085 Pitch")
                    self.sensor_data['roll'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.calculate_roll, "BNO085 Roll")
                    self.sensor_data['speed'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.calculate_speed, "BNO085 Speed")
                    self.sensor_data['quaternion'] = self.read_sensor_data(
                        self.sensors['bno085'], BNO085Sensor.read_bno085_quaternion, "BNO085 Quaternion")
                    self.sensor_data['solar'] = self.read_sensor_data(
                        self.sensors['ina3221'], lambda s: INA3221Sensor.read_ina3221(
                            s, 1), "INA3221 Solar")
                    self.sensor_data['battery'] = self.read_sensor_data(
                        self.sensors['ina3221'], lambda s: INA3221Sensor.read_ina3221(
                            s, 3), "INA3221 Battery")
                    self.sensor_data['battery_charge'] = self.read_sensor_data(
                        self.sensors['ina3221'], INA3221Sensor.battery_charge, "Battery Charge")
                    self.sensor_data['left_distance'] = self.read_sensor_data(
                        self.sensors['vl53l0x'], lambda s: VL53L0XSensors.read_vl53l0x(
    ...
    def ideal_mowing_conditions():
        ...
        # conditions
        if SensorInterface.read_bme280(
        )['temperature'] > 0 and SensorInterface.read_bme280()['humidity'] < 85:
            return True
        else:
            return False
